...
func getExtDynaTypeGoLang(
	dynaConfigStruct string,
	getDynaTableFn string,
	getDynaQueryFn string,
) string {
	return fmt.Sprintf(
		extDynaTypeGoLang,
		dynaConfigStruct,
...
const extDynaTypeGoLang = `package sqlc
...
func %s(query string) (dynaTb string, has bool) {
...
func %s(query string) (qMap map[string]string) {
	// cash_inex
	qMap = map[string]string{}

	dynaTableName, has := %s(query)
	if !has {
		return
	}
